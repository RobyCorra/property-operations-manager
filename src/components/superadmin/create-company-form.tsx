...
import { createCompanyWithManager } from "@/src/app/actions/superadmin";
import Link from "next/link";

export default function CreateCompanyForm() {
  const [open, setOpen] = useState(false);
  const [state, formAction, isPending] = useActionState(createCompanyWithManager, null);
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
          <div className="w-full max-w-md bg-slate-900 border border-slate-700 rounded-2xl p-6 space-y-5">
            ...
            <div className="flex items-center justify-between">
              <h2 className="text-base font-black text-white">Crea nuova impresa</h2>
            ...
            {state?.success ? (
              <div className="space-y-4">
                <div className="rounded-xl bg-emerald-500/10 border border-emerald-500/20 p-4 space-y-1">
                  <p className="text-sm font-bold text-emerald-400">✅ Impresa creata!</p>
                  <p className="text-xs text-slate-400">Nome: <span className="text-white font-semibold">{state.companyName}</span></p>
                </div>
                <div className="flex gap-2">
                  <Link
                    href={`/superadmin/company/${state.companyId}`}
                    className="flex-1 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold text-center transition-all"
              ...
              <form action={formAction} className="space-y-4">
                ...
                <div className="space-y-3">
                  <p className="text-[10px] font-bold uppercase tracking-widest text-slate-500">Impresa</p>
                  <input
                    required name="companyName" type="text" placeholder="Es. Impresa Pulizie Beta"
                    className="w-full rounded-xl bg-slate-800 border border-slate-600 px-4 py-2.5 text-white text-sm placeholder-slate-500 outline-none focus:ring-2 focus:ring-emerald-500"
                  />
                  <input
                    name="vatNumber" type="text" placeholder="Partita IVA (opzionale)"
                    className="w-full rounded-xl bg-slate-800 border border-slate-600 px-4 py-2.5 text-white text-sm placeholder-slate-500 outline-none focus:ring-2 focus:ring-emerald-500"
                  />
                </div>
                ...
                <div className="flex gap-2 pt-1">
                  ...
                  >
                    {isPending ? "Creazione..." : "Crea impresa"}
                  </button>
                  <button
