...
export default function CreateManagerForm({ orgId }: { orgId: string }) {
  ...
  if (state?.success) {
    return <p className="text-sm text-emerald-400 font-medium">✅ Manager creato con successo!</p>;
  }
  ...
  return (
    <form action={formAction} className="space-y-3">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <input
          required name="name" type="text" placeholder="Nome completo"
        ...
        />
        <input
          required name="password" type="password" placeholder="Password (min 8 char)" minLength={8}
