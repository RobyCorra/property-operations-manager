            ...
            <div className="md:hidden divide-y divide-gray-50">
              {activities.map(activity => (
                <div
                  ...
                  className="px-5 py-4 flex items-center gap-3 cursor-pointer active:bg-gray-50 transition-colors"
                  onClick={() => setSelectedActivity({ id: activity.id, type: activity.type })}
                >
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-bold text-gray-900 truncate">{activity.apartmentName}</p>
                    <p className="text-xs text-gray-400 mt-0.5">{formatRomeDateTimeDisplay(activity.date)}</p>
                  </div>
                  <div className={`shrink-0 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-tighter border ${STATUS_COLORS[activity.status as keyof typeof STATUS_COLORS]}`}>
