...
import { CheckCircle2, Circle, SkipForward, Camera, AlertTriangle, X } from "lucide-react";
    ...
    <div className="rounded-2xl border border-slate-100 bg-slate-50 p-5 space-y-4">
      ...
      <div className="space-y-2">
        {items.map((item) => {
          ...
          const label = pickLabel(item.label, item.labelTranslations, lang);
          return (
            <div
              key={item.id}
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                <p className={`text-sm font-medium ${item.completed ? "text-slate-800" : "text-slate-500"}`}>{label}</p>
              ...
              {/* Foto: miniatura cliccabile o badge "mancante" */}
              {item.photoUrl ? (
                <button
                  type="button"
                  onClick={() => setLightbox({ url: item.photoUrl!, label })}
                ...
                >
                  ...
                  <img src={item.photoUrl} alt={label} className="h-full w-full object-cover" />
                </button>
              ) : missingPhoto ? (
      ...
      {/* Approva / Richiedi correzioni — pannello supervisor riusato */}
      <div className="pt-1">
