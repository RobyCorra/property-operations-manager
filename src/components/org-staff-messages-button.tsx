...
export default function OrgStaffMessagesButton({ initialUnread, variant }: { initialUnread: number; variant: "desktop" | "mobile" }) {
  ...
  useEffect(() => {
    let alive = true;
    const check = async () => {
      const n = await getMyOrgStaffUnread();
      if (!alive) return;
  ...
  if (variant === "mobile") {
    return (
      <Link href="/dashboard/messaggi-org" className={`flex flex-1 flex-col items-center justify-center gap-1 py-3 ${unread > 0 ? "text-rose-600" : "text-slate-400 hover:text-violet-600"}`}>
