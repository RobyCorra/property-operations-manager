    ...
    <main className="min-h-screen bg-[#faf8ff] p-6 pb-28 font-sans text-slate-900 lg:p-10 lg:pb-10">
      <div className="max-w-4xl mx-auto space-y-10">
          ...
          <div>
            <div className="flex items-center gap-3">
              <h1 className="text-4xl font-semibold tracking-tight text-slate-900 uppercase">
                {t.svReviews} <span className="text-yellow-500">.</span>
              </h1>
              <div className="md:hidden"><OperativeLangPill variant="light" /></div>
            </div>
            <p className="text-slate-500 text-sm mt-1 font-medium">{t.moHello}, {user.name} {t.svHelloSuffix}</p>
          </div>
        ...
        {role === "SUPERVISOR" && (
          <section className="space-y-3">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-sm font-semibold uppercase tracking-widest text-slate-700">{t.svAssignedApartments}</h2>
            ...
            {assignedApartments.length === 0 ? (
              <div className="rounded-[2rem] border border-dashed border-slate-200 bg-white/30 p-8 text-center">
                <p className="text-sm text-slate-400 font-medium">{t.svNoApartments}<br />{t.svContactManager}</p>
              </div>
            ) : (
        ...
        {totalToReview === 0 && (
          <div className="bg-white/40 backdrop-blur-xl p-20 rounded-[3rem] border border-slate-200 border-dashed text-center">
            ...
            <h3 className="text-2xl font-semibold text-slate-900 tracking-tight uppercase">{t.svAllReviewed}</h3>
            <p className="text-slate-500 text-sm mt-2 font-medium">{t.svNothingToReview}</p>
          </div>
        )}
        ...
        {cleaningTasks.length > 0 && (
          <section className="space-y-4">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-sm font-semibold uppercase tracking-widest text-slate-700">{t.svCleaningsPending}</h2>
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="min-w-0 flex-1">
                      <div className="mb-2 flex flex-wrap items-center gap-2">
                        ...
                        {!task.assignedTo && (
                          <span className="rounded-full bg-rose-100 border border-rose-200 px-3 py-1 text-[9px] font-black uppercase tracking-widest text-rose-700">{t.svUnassignedF}</span>
                        )}
                      </div>
                      ...
                      <p className="mt-1 text-xs font-medium text-slate-500">
                        ...
                        {task.booking?.guestName && <span className="ml-2 text-slate-400">· {t.svGuestLabel}: {task.booking.guestName}</span>}
                      </p>
                    </div>
        ...
        {ticketsToReview.length > 0 && (
          <section className="space-y-4">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-sm font-semibold uppercase tracking-widest text-slate-700">{t.svMaintenancePending}</h2>
        ...
        {correctionPendingTasks.length > 0 && (
          <section className="space-y-4">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-sm font-semibold uppercase tracking-widest text-slate-700">{t.svCleaningsCorrections}</h2>
        ...
        {ticketsCorrectionPending.length > 0 && (
          <section className="space-y-4">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-sm font-semibold uppercase tracking-widest text-slate-700">{t.svMaintenanceCorrections}</h2>
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-50 flex items-stretch border-t border-slate-200 bg-white/95 backdrop-blur-md md:hidden safe-bottom">
        <div className="flex flex-1 flex-col items-center justify-center gap-1 py-3 text-yellow-500 relative">
          ...
          <span className="text-[9px] font-black uppercase tracking-widest">{t.svReviews}</span>
          {totalToReview > 0 && (
