    ...
    <main className="min-h-screen bg-[#faf8ff] p-6 pb-10 font-sans text-slate-900 lg:p-10">
      <div className="max-w-2xl mx-auto space-y-8">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-semibold uppercase tracking-tight text-slate-900">{t.svCleaningReview}</h1>
            <p className="text-xs font-medium text-slate-500 mt-0.5">{task.apartment.name} · {formatRomeDateTimeDisplay(task.date, t.moAt)}</p>
          </div>
        </div>

        {/* Info Card */}
        <div className="rounded-[2rem] border border-white/60 bg-white/55 p-5 shadow-lg backdrop-blur-xl space-y-4">
          <h2 className="text-xs font-black uppercase tracking-widest text-slate-400">{t.svInterventionDetails}</h2>
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            ...
            <div className="flex items-center gap-3 rounded-2xl bg-slate-50 p-3">
              ...
              <div>
                <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">{t.aiFDate}</p>
                <p className="text-sm font-semibold text-slate-900">{formatRomeDateTimeDisplay(task.date, t.moAt)}</p>
                {task.startedAt && <p className="text-xs text-slate-500">{t.svStart} {formatRomeDateTimeDisplay(task.startedAt, t.moAt)}</p>}
              </div>
            </div>
            <div className="flex items-center gap-3 rounded-2xl bg-slate-50 p-3">
              <User size={14} className="text-slate-400 shrink-0" />
              <div>
                <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">{t.svCleaner}</p>
                <p className="text-sm font-semibold text-slate-900">{task.assignedTo?.name ?? t.svUnassignedM}</p>
              </div>
            </div>
            {task.booking && (
              <div className="flex items-center gap-3 rounded-2xl bg-slate-50 p-3">
                <User size={14} className="text-slate-400 shrink-0" />
                <div>
                  <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">{t.svCheckoutGuest}</p>
                  <p className="text-sm font-semibold text-slate-900">{task.booking.guestName ?? "—"}</p>
                  {task.booking.totalGuests && <p className="text-xs text-slate-500">{task.booking.totalGuests} {t.svGuests}</p>}
                </div>
              </div>
            )}
          </div>
                      ...
                      <div className="flex-1 min-w-0 space-y-1">
                        ...
                        <div className="flex items-center gap-2 flex-wrap pt-1">
                          {item.requiresPhoto && (
                            <span className="flex items-center gap-1 rounded-full bg-amber-100 border border-amber-200 px-2 py-0.5 text-[9px] font-black uppercase tracking-widest text-amber-700">
                              <Camera size={9} /> {t.svPhotoRequired}
                            </span>
                          )}
                          <span className={`rounded-full px-2 py-0.5 text-[9px] font-black uppercase tracking-widest ${item.completed ? "bg-emerald-100 text-emerald-700" : "bg-rose-100 text-rose-600"}`}>
                            {item.completed ? t.svMarkedResolved : t.svNotResolved}
                          </span>
                        </div>
                        ...
                        {item.photoUrl && (
                          <a href={item.photoUrl} target="_blank" rel="noopener noreferrer" className="mt-2 inline-block group">
                            ...
                            />
                            <p className="mt-1 text-[10px] font-semibold text-emerald-600">{t.svTapToEnlarge}</p>
                          </a>
                        )}
                        {item.requiresPhoto && !item.photoUrl && (
                          <p className="text-[10px] font-semibold text-rose-500 pt-1">{t.svPhotoNotUploaded}</p>
                        )}
                      </div>
                    </div>
                  </div>
        ...
        <div className="rounded-[2rem] border border-white/60 bg-white/55 p-5 shadow-lg backdrop-blur-xl space-y-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-sm font-semibold uppercase tracking-tight text-slate-900">{t.svQualityChecklist}</h2>
            </div>
            <span className="rounded-full bg-slate-100 px-3 py-1 text-[10px] font-black uppercase tracking-widest text-slate-600">
              {completedCount}/{checklist.length} {t.svCompleted}
            </span>
          </div>

          {checklist.length === 0 ? (
            <p className="text-sm font-medium text-slate-400 italic">{t.svNoChecklist}</p>
          ) : (
              ...
              {checklist.map((item) => (
                ...
                >
                  ...
                  {item.required && !item.completed && (
                    <span className="shrink-0 rounded-full bg-rose-100 px-2 py-0.5 text-[9px] font-black uppercase tracking-widest text-rose-600">{t.svRequired}</span>
                  )}
                </div>
              ))}
            </div>
