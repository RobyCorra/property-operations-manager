...
export default async function ImpresaAppartamentoPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-4">
      ...
      {/* Riepilogo pulizie prossime */}
      {cleanings.filter((c) => c.status === "PENDING" || c.status === "IN_PROGRESS").length > 0 && (
        <section className="rounded-2xl border border-gray-100 bg-white p-4 shadow-sm">
          ...
          <div className="space-y-2">
            {cleanings
              ...
              .map((c) => (
                ...
                >
                  <span className={`w-2 h-2 rounded-full shrink-0 ${c.status === "IN_PROGRESS" ? "bg-violet-500" : "bg-amber-400"}`} />
                  <span className="flex-1 truncate text-slate-700">
                    {new Date(c.date).toLocaleDateString("it-IT", { day: "2-digit", month: "short", hour: "2-digit", minute: "2-digit" })}
                  </span>
                  <span className="text-[11px] text-slate-400">
                    {c.assignedTo?.name ?? "Non assegnata"}
                  </span>
