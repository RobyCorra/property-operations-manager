...
import { getMaintenanceLang, getMaintenanceT, MAINTENANCE_LANG_COOKIE } from "@/src/lib/server-lang";
import {
  LogOut,
  Navigation,
  CalendarDays,
...
export default async function MaintenanceDashboardPage({
  ...
  const priorityLabel: Record<string, string> = {
    LOW: t.moPrioLow,
    MEDIUM: t.moPrioMedium,
    HIGH: t.moPrioHigh,
    URGENT: t.moPrioUrgent,
  };
  ...
  return (
    ...
    <main className="min-h-screen bg-[#faf8ff] p-6 pb-28 font-sans text-slate-900 lg:p-10 lg:pb-10">
      <div className="max-w-6xl mx-auto space-y-12">
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div>
            <div className="flex items-center gap-3 mb-1">
              <h1 className="text-4xl font-semibold tracking-tight text-slate-900 uppercase">
                {t.moHello}, {user.name} <span className="text-violet-600">.</span>
              </h1>
              <div className="md:hidden"><OperativeLangPill variant="light" /></div>
            </div>
            <p className="text-slate-500 text-sm mt-1 font-medium tracking-normal">
              {isHistoryView ? t.moSubHistory : t.moSubOpen}
            </p>
          </div>
          ...
          <div className="hidden md:flex items-center gap-4">
            ...
            >
              <ScrollText size={14} />
              {isHistoryView ? t.moOpenTickets : t.moHistory}
            </Link>
        ...
        <section className="space-y-6">
          <div className="flex items-center gap-3 mb-2">
            <h2 className="text-lg font-semibold text-slate-900 tracking-tight uppercase">
              {isHistoryView ? t.moHistoryTickets : t.moOpenTickets}
            </h2>
          ...
          <div className="grid grid-cols-1 gap-6">
                    ...
                    <div className="min-w-0 space-y-2">
                      ...
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <div className="flex items-center gap-2 rounded-full bg-slate-100/70 px-4 py-1.5 text-[10px] font-bold uppercase tracking-widest text-slate-500">
                          ...
                          {ticket.status === "PENDING" ? t.moStPending : ticket.status === "IN_PROGRESS" ? t.moStInProgress : t.moStResolved}
                        </div>
                      </div>
                    ...
                    <>
                      ...
                      <div className="grid grid-cols-1 gap-5 lg:grid-cols-2">
                        ...
                        <div className="rounded-3xl border border-slate-100 bg-white/70 p-5 shadow-sm">
                          <p className="mb-4 text-[10px] font-black uppercase tracking-widest text-slate-400">{t.moTimes}</p>
                          <div className="space-y-2 rounded-2xl bg-slate-50 p-4">
                            ...
                            {ticket.scheduledStart ? (
                              ...
                              <p className="text-xs font-medium uppercase tracking-tight text-slate-400">{t.moNotYetPlanned}</p>
                            )}
                      ...
                      <div className="rounded-3xl border border-slate-100 bg-white/70 p-5 shadow-sm">
                        <div className="mb-4 flex items-center gap-2">
                          ...
                          <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">{t.moIntervention}</p>
                        </div>
                        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                          <div className="rounded-2xl bg-slate-50 p-4">
                            <p className="text-[9px] font-black uppercase tracking-widest text-slate-400">{t.moProblem}</p>
                            <p className="mt-1 text-sm font-bold text-slate-900">{ticket.title}</p>
                          </div>
                          <div className="rounded-2xl bg-slate-50 p-4">
                            <p className="text-[9px] font-black uppercase tracking-widest text-slate-400">{t.moDescNotes}</p>
                            <p className="mt-1 whitespace-pre-wrap text-sm font-medium leading-relaxed text-slate-700">
                              {ticket.description || t.moNoDesc}
                            </p>
                          </div>
                        </div>
                      </div>
                      ...
                      {ticket.status === "IN_PROGRESS" && Array.isArray(ticket.maintenanceTasks) && (ticket.maintenanceTasks as MaintenanceTaskItem[]).length > 0 && (
                        <div className="rounded-3xl border border-slate-100 bg-white/70 p-5 shadow-sm">
                          <div className="mb-4 flex items-center gap-2">
                            ...
                            <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">{t.moTasks}</p>
                          </div>
                      ...
                      <div className="rounded-3xl border border-slate-100 bg-white/70 p-5 shadow-sm">
                        <div className="mb-3 flex items-center gap-2">
                          <span className="text-lg">📎</span>
                          <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">{t.moAttachments}</p>
                        </div>
                        {ticketAttachments.length > 0 ? (
                          ...
                          <p className="rounded-2xl bg-slate-50 px-4 py-3 text-sm font-medium text-slate-400">{t.moNoAttachments}</p>
                        )}
                      </div>
                    </>
                  )}
            ...
            {user.maintenanceTickets.length === 0 && (
              <div className="bg-white/40 backdrop-blur-xl p-20 rounded-[3rem] border border-slate-200 border-dashed text-center">
                ...
                <h3 className="text-2xl font-semibold text-slate-900 tracking-tight uppercase">{t.moEmptyTitle}</h3>
                <p className="text-slate-500 text-sm mt-2 font-medium tracking-normal mb-10">
                  {isHistoryView ? t.moEmptyHistory : t.moEmptyOpen}
                </p>
                <Link
                  href={isHistoryView ? "/dashboard/maintenance" : "/dashboard/maintenance?view=history"}
                ...
                >
                  <ScrollText size={14} />
                  {isHistoryView ? t.moBackToOpen : t.moSeeHistory}
                </Link>
              </div>
            )}
          </div>
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-50 flex items-stretch border-t border-slate-200 bg-white/95 backdrop-blur-md md:hidden safe-bottom">
        ...
        >
          <Wrench size={20} />
          <span className="text-[9px] font-black uppercase tracking-widest">{t.moNavOpen}</span>
        </Link>
        <Link
        ...
        >
          <ScrollText size={20} />
          <span className="text-[9px] font-black uppercase tracking-widest">{t.moNavHistory}</span>
        </Link>
        <form action={logoutAction} className="flex flex-1">
          <button type="submit" className="flex flex-1 flex-col items-center justify-center gap-1 py-3 text-slate-400 hover:text-rose-500">
            <LogOut size={20} />
            <span className="text-[9px] font-black uppercase tracking-widest">{t.moNavLogout}</span>
          </button>
        </form>
      </nav>
    </main>
